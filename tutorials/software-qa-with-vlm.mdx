...
Build a custom TaskerAgent that automates UI testing by clicking through every sidebar button in an application and using a Vision Language Model (VLM) to verify each screen. We'll assemble the workflow piece by piece—extending TaskerAgent with custom validation logic.
...
2. **Python ≥ 3.10 with desktop extras**:
   ...
   ```
3. **A VLM for validation** – This tutorial uses a custom `ModelEngine` class. You can substitute with any vision model (GPT-4V, Gemini, Claude, etc.).
4. **Nuclear Player** – We'll use [Nuclear](https://github.com/nukeop/nuclear), an open-source streaming music player built with Electron. Install and run it locally so the agent can target its UI:
   ```bash
...
- `nuclear_qa.py` – main script
- `model_engine.py` – your VLM wrapper
- `apis/gemini.json` – model configuration
- `results/` – destination for screenshots and HTML transcripts
...
### Step 3 — Extend TaskerAgent with QA logic
...
The key addition is the VLM validation after each todo—the agent takes a screenshot and asks the VLM to confirm the expected page is displayed.

### Step 4 — Define the QA workflow
...
### Step 5 — Initialize and configure the agent
...
### Step 7 — Export the execution history
...
- **Console summary** – Displays per-todo success with emoji indicators and VLM validation results
- **QA results** – Shows whether each page was correctly identified by the VLM
- **HTML export** – Stored under `results/<exp_name>/nuclear_qa_execution_history.html` with screenshots for every step
...
1. **Update the todos** – Replace sidebar button names with your app's UI elements
2. **Update the checkers** – Match each todo with the expected page/state name
3. **Adjust VLM prompts** – Customize the validation questions for your UI

This pattern works for any desktop application with navigable UI elements—settings panels, menu systems, wizard flows, etc.

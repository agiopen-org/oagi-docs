...
2. **Python ≥ 3.10**:
   ...
   ```
3. **Nuclear Player** – We'll use [Nuclear](https://github.com/nukeop/nuclear), an open-source streaming music player built with Electron. Install and run it locally so the agent can target its UI:
   ```bash
...
- `results/` – destination for HTML execution history
...
### Step 3 — Initialize and configure the agent
...
### Step 5 — Export the execution history
...
- **HTML export** – Full execution history with screenshots at `results/<exp_name>/nuclear_qa_execution_history.html`
...
2. **Be specific** – Include location hints like "in the left sidebar" or "at the top of the page"
3. **Add more todos** – Test additional flows like settings panels, menu systems, or wizard flows
